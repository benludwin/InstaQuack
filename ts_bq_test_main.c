...
int EmptyTest(TSBoundedQueue *queue,int expected)
{
        int count = TS_BB_GetCount(queue);
        int empty = TS_BB_IsEmpty(queue);
...
int FullTest(TSBoundedQueue *queue,int expected)
{
        int count = TS_BB_GetCount(queue);
        int full = TS_BB_IsFull(queue);
...
int ValidIDTest(TSBoundedQueue *queue,long long id,int expected)
{

        long long front = TS_BB_GetFront(queue);
        long long back = TS_BB_GetBack(queue);
        int valid =TS_BB_IsIdValid(queue,id); 
...
void *GetItemTest(TSBoundedQueue *queue,long long id,int expected)
{
...
long long EnqueTest(TSBoundedQueue *queue,void *item,int expected)
{
        long long id = TS_BB_TryEnqueue(queue,item); 
...
int DequeueTest(TSBoundedQueue *queue,long long id,int expected)
{

        int deq = TS_BB_TryDequeue(queue,id);
...
int CountEqualTest(TSBoundedQueue *queue,int test_count,int expected)
{
        int queue_count = TS_BB_GetCount(queue);
...
void SimpleOrderTest(struct FileLines* Lines)
{
        ...
        queue = TS_BB_MallocBoundedQueue(mid_point);
...
int main(int argc, char *argv[])
{
        ...
        queue = TS_BB_MallocBoundedQueue(Lines->LineCount);
