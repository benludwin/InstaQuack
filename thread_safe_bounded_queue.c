...
long long TS_BB_TryEnqueue(struct thread_safe_bounded_queue *queue,void *item)
{
	...
	fprintf(stderr, "Entering critical section of Enqueue\n");
        returnValue = BB_TryEnqueue(queue->queue,item);
	fprintf(stderr, "Exiting critical section of Enqueue\n");
...
int TS_BB_TryDequeue(struct thread_safe_bounded_queue *queue,long long id)
{
        int  returnValue = 0;
        pthread_mutex_lock(&(queue->lock)); 
	fprintf(stderr, "Entering critical section of Dequeue\n");
        returnValue = BB_TryDequeue(queue->queue,id);
	fprintf(stderr, "Exiting critical section of Dequeue\n");
        pthread_mutex_unlock(&(queue->lock)); 
        return returnValue;
}
